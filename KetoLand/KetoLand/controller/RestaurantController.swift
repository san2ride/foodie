...
class RestaurantController: UIViewController {
    ...
    @IBAction func didTapSave(_ sender: UIBarButtonItem) {
        let alertController = UIAlertController(title: "Save Keto List", message: "", preferredStyle: UIAlertController.Style.alert)
        ...
        let defaultAction = UIAlertAction(title: "Save", style: UIAlertAction.Style.default) { [weak self] (action: UIAlertAction) in
            ...
            self?.ketoService.saveCurrentList(withName: name)
        }
    ...
    private var ketoService = KetoService.shared
        ...
        LocationService.shared.locationUpdateHandler = { [weak self] (needUpdate: Bool, userLocation: CLLocation?) in
            if needUpdate {
                ...
                DispatchQueue.main.async {
                    if let currentPlaces = self?.ketoService.places, currentPlaces.count > 0 {
    ...
    private func fetchRestaurantList() {
        ketoService.loadRestaurants(around: userLocation) { [weak self] (arrPlaces: [Place]?) in
...
extension RestaurantController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "ketoCell", for: indexPath) as! RestaurantCell
